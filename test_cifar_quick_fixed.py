"""
QUICK CIFAR-10 continual learning test (completes in 30 mins) - FIXED.

Uses explicit step limits instead of FLOP calculation to avoid estimation bugs.
"""
...
def main():
    config = override_config(CONFIG, {
        ...
        # Training data
        "cifar_max_train_per_class": 100,  # 100 images per class
        "cifar_max_test_per_class": 50,    # 50 test images per class

        # Training budget - FIXED: Use explicit step limit
        "_max_training_steps": 2000,        # 2000 steps per task (reasonable for learning)
        "flops_target": 1e12,               # Set very high so step limit is used
    ...
    print("=" * 70)
    print("QUICK CIFAR-10 Continual Learning Test (30 mins) - FIXED")
    print("=" * 70)
    ...
    print(f"Total training runs: 5 tasks × 3 variants = 15")
    print(f"Estimated total steps: ~{config['_max_training_steps'] * 5 * 3:,}")
    print()
    print("🔧 FIX: Direct step limit bypasses broken FLOP estimation")
    print()
    print("This tests:")
